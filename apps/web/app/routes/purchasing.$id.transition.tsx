...
import type { Route } from "./+types/purchasing.$id.transition";
import { requireUser } from "~/lib/auth.server";
import { queries } from "~/lib/db.server";
import { poTransitionSchema } from "@brewplan/shared";
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  const result = poTransitionSchema.safeParse(raw);
  if (!result.success) {
    const errorMsg = result.error.issues.map((i) => i.message).join(", ");
    return redirect(
      `/purchasing/${params.id}?error=${encodeURIComponent(errorMsg)}`
    );
  }

  try {
    queries.purchasing.transition(params.id, result.data.toStatus);
  } catch (error) {
    const message =
